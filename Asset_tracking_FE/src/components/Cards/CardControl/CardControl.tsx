...
function CardControl({ socket }: Props) {
  ...
  useEffect(() => {
    ...
    socket.on('response_status_pump_from_esp32', (data) => {
      if (data.status === '0' || data.status === '1') setPump(data.status)
    })
    ...
    const handleKeyDown = (event: KeyboardEvent) => {
      ...
      } else if (keyBoard.stop.includes(pressedKey)) {
        setBtnActive(direction.stop)
      }
    }
    ...
    const handleKeyUp = () => {
      if (!isConnected) return
      setBtnActive(null)
      socket.emit('send_direction', direction.stop)
    }
  ...
  return (
    <div className='relative mb-6 flex w-full min-w-0 flex-col break-words rounded bg-white shadow-lg'>
      ...
      <div className='flex flex-auto flex-col p-4'>
        ...
        {/* controls pump */}
        <Button
          className={classNames('bg-xanh rounded px-6 py-3 font-medium uppercase text-white hover:shadow')}
